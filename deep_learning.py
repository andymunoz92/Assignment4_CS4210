...
def build_model(n_hidden, n_neurons_hidden, n_neurons_output, learning_rate):
    ...
    nn = keras.models.Sequential() ## nn for neural network
    ...
    for _ in range(1, n_hidden + 1):
        nn.add(keras.layers.Dense(n_neurons_hidden, activation="relu"))
    ...
    nn.add(keras.layers.Dense(n_neurons_output, activation="softmax"))
...
for h in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for n in n_neurons:                     #looking or the best parameters w.r.t the number of neurons
        for l in l_rate:                    #looking or the best parameters w.r.t the learning rate
            ...
            model = build_model(h, n, n_classes, l)
            ...
            #-->add your Pyhton code here
            accuracy = metrics.accuracy_score(y_test, class_predicted)
            if accuracy > ha:
                ha = accuracy
                print("Highest accuracy so far: " + str(ha))
